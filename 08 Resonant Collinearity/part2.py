...
def find_antinodes(ant1, ant2, unique_locations, city_limits):
    ...
    dy = ant1[1] - ant2[1]
    factor = 0
    out_of_bounds_1 = False
    out_of_bounds_2 = False
    
    while not (out_of_bounds_1 and out_of_bounds_2):
        if not out_of_bounds_1:
            new_x = ant1[0] + dx * factor
            new_y = ant1[1] + dy * factor
            if new_x >=0 and new_x < city_limits[0] and new_y >=0 and new_y < city_limits[1]:
                unique_locations.add((new_x, new_y))
            else:
                out_of_bounds_1 = True
        if not out_of_bounds_2:
            new_x = ant2[0] - dx * factor
            new_y = ant2[1] - dy * factor
            if new_x >=0 and new_x < city_limits[0] and new_y >=0 and new_y < city_limits[1]:
                unique_locations.add((new_x, new_y))
            else:
                out_of_bounds_2 = True
        factor += 1
